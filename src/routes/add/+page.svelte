...
<section class="add-shell">
	...
	<div class="add-grid">
		<div class="add-card">
			{#if data?.error}
				<p class="form-error">⚠️ {data.error}</p>
			{/if}
			...
			<form method="POST" action="?/create" class="form-grid">
				...
				<label class="field">
					<span>📅 Mahdollinen tapaaminen</span>
					<input
						type="datetime-local"
						name="meetingAt"
				...
				<label class="field">
					...
					<textarea name="notes" rows="5" placeholder="Muistiinpanot, kysymykset, fiilikset...">{data?.notes ?? ''}</textarea>
				</label>
